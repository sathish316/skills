...
CMUX_FANOUT_AGENT_NAME="claude code"
CMUX_FANOUT_BINARY=${CMUX_FANOUT_BINARY:-claude}
CMUX_FANOUT_TITLE_PREFIX=claude
...
cmux_fanout_agent_command() {
  ...
  [[ -z "$profile" ]] || die "Claude Code wrapper does not support --profile"
  if [[ "$mode" == "daemon" ]]; then
    cmd+=" --print"
  fi
  if [[ -n "$model" ]]; then
